...
const textArray = ["Hello...!", "I'm Chandika Purna Revanth", "AI/ML Enthusiast", "Python Developer", "SIH 2024 Finalist"];
let wordIndex = 0;
let charIndex = 0;

function typeWord() {
if (charIndex < textArray[wordIndex].length) {
    typingEffect.textContent += textArray[wordIndex].charAt(charIndex);
    charIndex++;
    setTimeout(typeWord, 100); // Adjust speed here
} else {
    setTimeout(deleteWord, 1000); // Wait before deleting word
}
}

function deleteWord() {
if (charIndex > 0) {
    typingEffect.textContent = textArray[wordIndex].substring(0, charIndex - 1);
    charIndex--;
    setTimeout(deleteWord, 50); // Speed of deleting
} else {
    wordIndex = (wordIndex + 1) % textArray.length; // Loop through textArray
    setTimeout(typeWord, 500); // Delay before starting next word
}
}
...
function flipCard(card) {
    ...
    function openImageModal(imageSrc) {
        ...
        modal.style.display = "flex";  // Ensures full visibility
    }
